		...
		$html .= '<div class="casestudy-image-slider"><p class="new-heading">Wayfinding</p>';
			while($query->have_posts()){
				...
				$html .= '<div data-stellar-background-ratio="0.2" class="image-slide" style="background-image:url('.$slider_big_image['url'].')">
								...
								<div class="avia-button-wrap avia-button-center avia-builder-el-no-sibling"><a href="https://gomowebb.com/focusneo/case/" class="avia-button   avia-icon_select-no avia-size-medium avia-position-center avia-color-theme-color"><span class="avia_iconbox_title">See more case</span></a></div>
						  </div>';
					}
			wp_reset_postdata();
		$html .= '</div>';
	...
	if($query->have_posts()){
		...
		$terms = get_the_terms( $post->ID, 'casestudy_category' );
			if ( ! empty( $terms ) ) {
				...
				$html .= '<p class="heading_cat"><img src="https://gomowebb.com/focusneo/wp-content/uploads/2022/12/right-path.png" width ="30" alt="" / >'.$term->name.'</p>';
				$html .= '</div>';
			}
		return $html;
	}
...
function get_next_news(){
	...
	if($query->have_posts()){
		...
		if($next_post) {
			$html .= '<a href="' . get_permalink($next_post->ID) . '" title="' . $next_post->post_title . '" class="next-post-btn">VIEW PREVIOUS ARTICLE <img width="9" src="https://gomowebb.com/focusneo/wp-content/uploads/2022/12/right-path.png" /></a>'; 
		}
	 else {
			$html .= '<a href="' . get_permalink($prev_post->ID) . '" title="' . $prev_post->post_title . '" class="next-post-btn">VIEW PREVIOUS ARTICLE <img width="9" src="https://gomowebb.com/focusneo/wp-content/uploads/2022/12/right-path.png" /></a>'; 
		} 
	return $html;
	}
}
...
function get_product_accordion(){
	ob_start();
	$args = array(
		'post_type' => array('casestudy','post', 'product'),
		'posts_per_page' => -1, 
		'post_status' => 'publish',
		'orderby' => 'title',
		'order'   => 'ASC',
