	...
	if($query->have_posts()){
		$html .= '<div class="casestudy-image-slider"><p class="new-heading">Wayfinding</p>';
			while($query->have_posts()){
				...
				$html .= '<div data-stellar-background-ratio="0.2" class="image-slide" style="background-image:url('.$slider_big_image['url'].')">
							<p class="sticky-client"><span class="client-name">KUND: </span><span class="client-display">'.get_field('client_name').'</span></p>
								<a href="'.get_the_permalink().'"><img src="'.$slider_small_image['url'].'" alt="'.$slider_small_image['alt'].'"  /> </a>
								<div class="avia-button-wrap avia-button-center avia-builder-el-no-sibling"><a href="https://gomowebb.com/focusneo-sv/case/" class="avia-button   avia-icon_select-no avia-size-medium avia-position-center avia-color-theme-color"><span class="avia_iconbox_title">Se fler case</span></a></div>
						  </div>';
					} 
			wp_reset_postdata();
		$html .= '</div>';
				...
				foreach( $images as $image ): 
					$html .= '<div class="swiper-slide">
								...
								<div class="floating-box"> 
									<span class="case-client"><span class="client-name"> Kund: </span><span class="client-display">'.get_field('client_name').'</span>
										</span>
...
function waybrand_case_client(){
	...
	if($query->have_posts()){
		$html .= '<div class="client-name">';
					$html .= '<h3 class="sticky"><span class="client-name"> Kund: </span><span class="client-display">'.get_field('client_name',$post_id).'</span></h3>
...
function case_detail_shortcode(){
	...
	if($query->have_posts()){
		$html .= '<div class="client-name">';
					$html .= '<p><b>Kund:</b><br />'.get_field('client_name',$post_id).'</p>';
					if( get_field('what',$post_id) ) {
					$html .= '<p><b>Vad:</b><br />'.get_field('what',$post_id).'</p>';
					}
					if( get_field('when',$post_id) ) {
					$html .= '<p><b>När:</b><br />'.get_field('when',$post_id).'</p>';
					}
					if( get_field('where',$post_id) ) {
					$html .= '<p><b>Var:</b><br />'.get_field('where',$post_id).'</p>';
					}
					if( get_field('agency',$post_id) ) {
					$html .= '<p><b>Designbyrå:</b><br />'.get_field('agency',$post_id).'</p>';
					}
...
function new_get_product(){
	ob_start();
	$args = array(
		'post_type' => array('casestudy','product'),
		'posts_per_page' => -1, 
		'post_status' => 'publish',
		'orderby' => 'title',
		'order'   => 'ASC',
	...
	$args = array(
		...
		$my_query = new WP_Query($args);
			...
			if ( $my_query->have_posts() ) {
				...
				$html .= '<p>Tyvärr, inget innehåll kunde finnas som matchade vad du sökte efter.</p>';
			}
			...
			foreach ( $post_groups as $post_type => $post_ids ) {
				...
				foreach ( $post_ids as $post_id ) {
					...
					if ( has_post_thumbnail($post->ID)) {
							...
							$html .= '<div><img src="https://gomowebb.com/focusneo-sv/wp-content/uploads/2023/03/placeholder.jpg"/>';
						}
